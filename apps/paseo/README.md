...
- **AgentRouter provider** — a Plexus-style virtual route with ordered **fast, planning, judgment, build, and browser** target groups. Haiku 4.5 is control-only; every answer runs as a concrete Paseo child, whose provider, model, status, and agent ID appear in that agent's **Routing** panel. Hidden provider subagents are labelled `Unknown (runtime not exposed)`. Claude/Codex account selection still goes through the existing health router. The static Paseo adapter means a total Claude-family outage before the control plane boots cannot yet jump to a different family; child targets can fail over cross-provider once it is running.
