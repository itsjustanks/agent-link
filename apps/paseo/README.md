...
- **AgentRouter provider** — the first Agent Link tab explains and configures the controller plus ordered target groups. Fable is the default interpreter; every answer runs as a concrete Paseo child. Targets may use any native, custom, or ACP provider. Claude/Codex account selection still goes through the existing health router. The controller must boot through one Claude-compatible Paseo adapter; child targets can fail over cross-provider once it is running.
- **Provider CLI updates** — the Accounts tab exposes the daily/manual schedule, safe run-now action, verified built-in recipes, and custom argument-by-argument recipes for every other provider Paseo lists. Live provider processes are skipped. This never performs a quota probe.
