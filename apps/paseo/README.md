...
One horizontally scrollable tab per enabled provider family — **Claude Code, Codex, Kimi Code, Grok, and custom additions**. A cheap 30-second heartbeat refreshes the Paseo daemon, provider registry, account readiness, routing decisions, limit sentry, and capacity snapshots without starting an agent or spending quota; **Deep check** is the explicit, provider-starting diagnostic. Claude/Codex account-pinned aliases roll into their family tab instead of cluttering the strip. Each pooled-provider tab contains its own health, usage and capacity, routing, then accounts; there are no duplicate global usage or routing panels. Inside those tabs:
...
- **Provider usage / Limits** — names the rolling session and weekly windows for every account, with used and available capacity, exact reset countdowns, reset clock time explicitly labelled as the viewing device's local time, freshness, plan/model context, extra credits, and whether routing considers it available, nearing, held, or cooling down. No token material is read.
