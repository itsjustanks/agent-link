...
- **Automatic routing row** — one click wires a single `Claude (Dynamic Agent Link)` / `Codex (Dynamic Agent Link)` provider. The first row labels the next **new launch** forecast separately from recent launch history, including each Paseo agent and project. Selection is deterministic: health gate → priority group → least-recently-used target. The model is inspired by [Plexus](https://github.com/mcowger/plexus) (MIT), adapted to process launches; a running agent is never re-routed, and an all-parked pool blocks cleanly instead of launching a known-dead primary.
- **Live model pill** — every agent composer shows the runtime model Paseo reports. AgentRouter changes from `routing…` to its concrete answer model; clicking the pill opens provider, model, status, and agent-ID evidence. Hidden models stay explicitly unknown.
- **AgentRouter provider** — an optional Plexus-style virtual route with ordered **fast, planning, judgment, build, and browser** target groups. It earns its extra Haiku hop only when you want one automatic entry point; direct Paseo profiles are faster when the target is already known. Every answer runs as a concrete Paseo child. Claude/Codex account selection still goes through the existing health router. The static Paseo adapter means a total Claude-family outage before the control plane boots cannot yet jump to a different family; child targets can fail over cross-provider once it is running.
