...
- **Model and account composer modal** — every agent's pill opens a native Paseo modal with the provider/model, current AgentLink sign-in, per-account limits and local reset times. Prioritise a sign-in for new chats, or move a stopped Claude/Codex chat to a chosen healthy account and continue. AgentRouter separates its request reader and answer model. Hidden values stay explicitly unknown.
