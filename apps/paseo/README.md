...
- **Memory guard tab** — live Paseo-owned TypeScript checks only. One runs at a time; under critical pressure it is paused, not killed, and continued after the machine recovers. Checks launched from Terminal are never touched, and recent guard actions are labelled as history.
...
- **AgentRouter provider** — installs as a normal Paseo provider backed by Haiku 4.5. Pick it in any workspace; it handles cheap triage itself and delegates larger work through Paseo, while Claude account selection still goes through the existing health router.
