...
grep -q 'codex-auto/gpt-5.6-luna · account: auto' "$fixture_root/router/policy.md"
...
grep -q 'if the user names a provider, model, or account, use exactly that' "$fixture_root/router/rules.md"
...
grep -q '"controllerAccount": "auto"' "$fixture_root/router/config.json"
...
data["controllerAccount"] = "primary"
...
grep -q 'controllerAccount' "$fixture_root/bin/agent-router"
