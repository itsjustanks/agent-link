...
try {
  ...
  assert.ok(invocations[0].input.includes("Read ~/.agents/skills/paseo/SKILL.md before using Paseo tools"));
  assert.ok(invocations[0].input.includes("Native subagents are appropriate for quick same-provider work"));
  assert.ok(invocations[0].input.includes("use Paseo subagents for cross-provider work"));
  assert.ok(invocations[0].input.includes("use the Paseo CLI only when agent-scoped"));
  assert.ok(invocations[0].input.includes("Prefer direct Paseo providers. AgentLink is a fallback"));
